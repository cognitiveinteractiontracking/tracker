...
AprilTagDetector::AprilTagDetector(ros::NodeHandle& nh, ros::NodeHandle& pnh): it_(nh){
  ...
  pnh.param<int>("pub_pixel", pubPixelMode, 0);
  ...
  image_pub_ = it_.advertise(pnh.getNamespace() + "/tag_detections_image", 1);
  detections_pub_ = nh.advertise<AprilTagDetectionArray>(pnh.getNamespace() + "/tag_detections", 1);
//  pose_pub_ = nh.advertise<geometry_msgs::PoseArray>(pnh.getNamespace() + "/tag_detections_pose", 1);
  pose_pub_pixel = nh.advertise<AprilTagDetectionArray>(pnh.getNamespace() + "/tag_detections_pixel", 1);
}
...
void AprilTagDetector::imageCb(const sensor_msgs::ImageConstPtr& msg, const sensor_msgs::CameraInfoConstPtr& cam_info){
  ...
  AprilTagDetectionArray tag_detection_array_pixel;
  BOOST_FOREACH(AprilTags::TagDetection detection, detections){
    ...
    if(pubPixelMode) {
      geometry_msgs::PoseStamped tag_pose_pixel = tag_pose;
      tag_pose_pixel.pose.position.x = detection.cxy.first;
      tag_pose_pixel.pose.position.y = detection.cxy.second;
      tag_pose_pixel.pose.position.z = 0;
      AprilTagDetection tag_detection_pixel = tag_detection;
      tag_detection_pixel.pose = tag_pose_pixel;
      tag_detection_array_pixel.detections.push_back(tag_detection_pixel);
    }

  }
  if(pubPixelMode)
    pose_pub_pixel.publish(tag_detection_array_pixel);
